...
- `main`へのcommitとpushは禁止し、すべての変更を作業ブランチからPull Request経由で取り込む。
- Pull Requestは最新の`main`を取り込み、必須CIを成功させ、レビュー会話をすべて解決してからsquash mergeする。
- リリース開始用の`vX.Y.Z`タグはpushできるが、公開済みタグを削除またはforce更新しない。
